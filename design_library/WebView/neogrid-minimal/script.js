...
class Knob {
  constructor(element) {
    ...
    this.cap = element.querySelector('.knob-cap-group');
  ...
  update() {
    // Arc Update
    if (this.path) {
        const circumference = 2 * Math.PI * 28; // Hardcoded r=28 from SVG
    ...
    // Rotation Update (for Analog style)
    if (this.cap) {
        const rotation = -135 + (this.value * 270);
        this.cap.setAttribute('transform', `rotate(${rotation} 32 32)`);
    }
  }
}
...
class Fader {
  ...
  onMouseMove(e) {
    if (!this.isDragging) return;
    const delta = this.startY - e.clientY;
    const pixelRange = this.element.clientHeight - 48; // handle height
  ...
  update() {
    if (this.handle) {
        this.handle.style.bottom = `calc(${this.value * 100}% - 24px)`;
    }
    if (this.fill) {
